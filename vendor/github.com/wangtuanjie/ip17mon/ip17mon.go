...
func (loc *Locator) Dump() (rs []Range, locs []*LocationInfo) {
	...
	rs = make([]Range, 0, len(loc.indexData1))
	locs = make([]*LocationInfo, 0, len(loc.indexData1))

	for i := 1; i < len(loc.indexData1); i++ {
		s, e := loc.indexData1[i-1], loc.indexData1[i]
		off := loc.indexData2[i]
		l := newLocationInfo(loc.textData[off : off+loc.indexData3[i]])
