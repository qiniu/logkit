...
func Test_Read(t *testing.T) {
	...
	sf, err := NewSeqFile(meta, Dir, false, false, []string{".pid"}, "*", WhenceOldest, true)
	if err != nil {
		t.Error(err)
	}
...
func Test_NewReaderWithQiniuLogFile(t *testing.T) {
	...
	sf, err := NewSeqFile(meta, Dir, false, false, []string{".pid"}, `logkit.log-*`, WhenceOldest, true)
	if err != nil {
		t.Error(err)
	}
	buffer := make([]byte, 8)
...
func Test_NewFileNewLine(t *testing.T) {
	...
	sf, err := NewSeqFile(meta, Dir, false, true, []string{".pid"}, `*`, WhenceOldest, true)
	if err != nil {
		t.Error(err)
	}
	buffer := make([]byte, 17)
...
func Test_NewReaderWithInvalidFile(t *testing.T) {
	...
	sf, err := NewSeqFile(meta, Dir, false, false, []string{".pid"}, `test-logkit.log-*ss`, WhenceOldest, true)
	if err != nil {
		t.Error(err)
	}
...
func Test_ReadNewest(t *testing.T) {
	...
	sf, err := NewSeqFile(meta, Dir, false, false, []string{".pid"}, "*", WhenceNewest, true)
	if err != nil {
		t.Error(err)
	}
	buffer := make([]byte, 5)
...
func Test_NewFileOffset(t *testing.T) {
	...
	sf, err := NewSeqFile(meta, dirPath, false, false, []string{".pid"}, "*", WhenceOldest, true)
	if err != nil {
		t.Error(err)
	}
	buffer := make([]byte, 5)
	...
	assert.Equal(t, "22345", string(buffer))

	AppendFiles(1000)

	_, err = sf.Read(buffer)
	assert.NoError(t, err)
	assert.Equal(t, "67891", string(buffer))
}
