...
var (
	...
	ModeToolTips = []KeyValue{
		...
		{ModeKafka, "Kafka reader 是logkit提供的从Kafka读取数据的配置方式。Kafka reader 输出的是raw data，可根据具体情况自定义parser进行解析。"},
...
var ModeKeyOptions = map[string][]Option{
	...
	ModeDirx: {
		...
		OptionKeyIgnoreFileSuffix,
		{
			...
			CheckRegex:   "\\d+[hms]",
			ToolTip:      `当文件夹内所有的日志达到expire时间，则放弃追踪。写法为：数字加单位符号，组成字符串duration写法，支持时h、分m、秒s为单位，类似3h(3小时)，10m(10分钟)，5s(5秒)，默认的expire时间是720h`,
		},
		OptionKeyMaxOpenFiles,
		OptionKeyStatInterval,
		OptionKeyValidFilePattern,
