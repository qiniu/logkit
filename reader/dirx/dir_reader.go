...
func (dr *dirReader) Run() {
	...
	for {
		...
		if len(dr.readcache) == 0 {
			...
			dr.readLock.Unlock()
			if err != nil && len(dr.readcache) != 0 && dr.runnerName == "ccfs_atest" {
				log.Errorf("Runner[%v] log path[%v] log file[%v] reader read failed, cache: %v, error: %v", dr.runnerName, dr.originalPath, dr.br.Source(), dr.readcache, err)
			}
