...
func NewSnmpReader(meta *Meta, c conf.MapConf) (s *SnmpReader, err error) {
	...
	if version == 3 {
		...
		secLevel, _ = c.GetStringOr(KeySnmpReaderSecLevel, "noAuthNoPriv")
		secName, _ = c.GetStringOr(KeySnmpReaderSecName, "user")
		authProtocol, _ = c.GetStringOr(KeySnmpReaderAuthProtocol, "MD5")
		authPassword, _ = c.GetStringOr(KeySnmpReaderAuthPassword, "pass")
		privProtocol, _ = c.GetStringOr(KeySnmpReaderPrivProtocol, "DES")
...
func (s *SnmpReader) gatherTable(gs snmpConnection, t Table, topTags map[string]string, walk bool) (data []map[string]interface{}, err error) {
	var rt *RTable
	...
	for _, tr := range rt.Rows {
		...
		d[KeyTimestamp] = rt.Time
		d[KeySnmpTableName] = t.Name
		data = append(data, d)
	}
	return
...
func (s *SnmpReader) getConnection(idx int) (snmpConnection, error) {
