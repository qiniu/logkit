...
func (rs *RestService) PostTransform() echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		var data = []Data{}              // multiple sample logs in map format
		var singleData Data              // single sample log in map format
		var bts []byte
		...
		if _, ok = reqConf[transforms.KeyType]; !ok {
			// param absence
			return RespError(c, http.StatusBadRequest, ErrTransformTransform, fmt.Sprintf("missing param %s", transforms.KeyType))
		}
		tp, ok = (reqConf[transforms.KeyType]).(string)
		if !ok {
			return RespError(c, http.StatusBadRequest, ErrTransformTransform, fmt.Sprintf("param %s must be of type string", transforms.KeyType))
		}
		create, ok := transforms.Transformers[tp]
		if !ok {
			// no such type transformer
			return RespError(c, http.StatusBadRequest, ErrTransformTransform, fmt.Sprintf("transformer of type %v not exist", tp))
		}
		// param 2: sample logs
		if _, ok = reqConf[KeySampleLog]; !ok {
			return RespError(c, http.StatusBadRequest, ErrTransformTransform, fmt.Sprintf("missing param %s", KeySampleLog))
		}
		...
		if jsonErr = jsoniter.Unmarshal([]byte(rawLogs), &singleData); jsonErr != nil {
			...
			if jsonErr = jsoniter.Unmarshal([]byte(rawLogs), &data); jsonErr != nil {
				...
				return RespError(c, http.StatusBadRequest, ErrTransformTransform, jsonErr.Error())
			}
		} else {
		...
		if transErr != nil {
			...
			return RespError(c, http.StatusBadRequest, ErrTransformTransform, fmt.Sprintf("transform processing error %v", transErr))
		}
