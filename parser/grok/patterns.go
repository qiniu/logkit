...
LOGKIT_CLOUDFRONT_ACCESS_LOG %{LOGKIT_CLOUDFRONT_ACCESS_LOG_TIME:timestamp}%{SPACE}%{NOTSPACE:x_edge_location}%{SPACE}(?:%{NUMBER:sc_bytes:int}|-)%{SPACE}%{IPORHOST:clientip}%{SPACE}%{NOTSPACE:cs_method}%{SPACE}%{HOSTNAME:cs_host}%{SPACE}%{NOTSPACE:cs_uri_stem}%{SPACE}%{NUMBER:sc_status:int}%{SPACE}%{NOTSPACE:referrer}%{SPACE}%{NOTSPACE:agent}%{SPACE}%{NOTSPACE:cs_uri_query}%{SPACE}%{NOTSPACE:cookies}%{SPACE}%{WORD:x_edge_result_type}%{SPACE}%{NOTSPACE:x_edge_request_id}%{SPACE}%{HOSTNAME:x_host_header}%{SPACE}%{URIPROTO:cs_protocol}%{SPACE}%{INT:cs_bytes:int}%{SPACE}%{GREEDYDATA:time_taken:float}%{SPACE}%{GREEDYDATA:x_forwarded_for}%{SPACE}%{GREEDYDATA:ssl_protocol}%{SPACE}%{GREEDYDATA:ssl_cipher}%{SPACE}%{GREEDYDATA:x_edge_response_result_type}
LOGKIT_CLOUDFRONT_ACCESS_LOG_TIME %{YEAR}-%{MONTHNUM}-%{MONTHDAY}%{SPACE}%{TIME}
...
LOGKITNETSCREENSESSIONLOG %{SYSLOGTIMESTAMP:date} %{IPORHOST:device} %{IPORHOST}:( NetScreen device_id=%{WORD:device_id}%{DATA}:)*( start_time=%{QUOTEDSTRING:start_time})*( duration=%{INT:duration})*( policy_id=%{INT:policy_id})*( service=%{DATA:service} proto=%{INT:proto})*( src zone=%{WORD:src_zone})*( dst zone=%{WORD:dst_zone})*( action=%{WORD:action} sent=%{INT:sent})*( rcvd=%{INT:rcvd})*( src=%{IPORHOST:src_ip})*( dst=%{IPORHOST:dst_ip})*( src_port=%{INT:src_port})*( dst_port=%{INT:dst_port})*( src-xlated ip=%{IPORHOST:src_xlated_ip})*( port=%{INT:src_xlated_port})*( dst-xlated ip=%{IPORHOST:dst_xlated_ip})*( port=%{INT:dst_xlated_port})*( session_id=%{INT:session_id})*( reason=%{GREEDYDATA:reason})*%{GREEDYDATA:message}
