...
注意，发送之前数据表必须已存在`,
	...
	TypeS3: {
		...
		},
		{
			...
			DefaultNoUse: false,
			Required:     true,
			Description:  "访问密钥ID(AK), 支持从自定义环境变量（如 YOUR_S3_AK_ENV）里读取对应值",
		},
		{
			...
			DefaultNoUse: false,
			Required:     true,
			Description:  "与访问密钥ID结合使用的密钥(SK), 支持从自定义环境变量（如 YOUR_S3_SK_ENV）里读取对应值",
		},
		{
			KeyName:      KeyS3Bucket,
			ChooseOnly:   false,
		...
		{
			KeyName:      KeyS3Pattern,
			ChooseOnly:   false,
			Default:      "",
			Placeholder:  "发送到指定文件",
			DefaultNoUse: false,
			Required:     true,
			Description:  "路径支持魔法变量，例如 \"pattern\":\"data-%Y-%m-%d.txt\" ，此时数据就会渲染出日期，存放为 data-2018-03-28.txt",
		},
		{
			KeyName:       KeyFileWriteRaw,
			ChooseOnly:    true,
		...
		{
			KeyName:      KeyS3MaxLines,
			ChooseOnly:   false,
			Default:      "",
			Placeholder:  "单个文件最多行数",
			DefaultNoUse: false,
			Required:     false,
			Description:  "写入文件的行数",
			Advance:      true,
		...
		},
		{
			...
			DefaultNoUse:  false,
			Required:      false,
			Description:   "发送的文件是否需要gzip压缩",
			Advance:       true,
		},
		{
			KeyName:      KeyS3SendInternal,
			ChooseOnly:   false,
			Default:      "",
			Placeholder:  "发送间隔",
			DefaultNoUse: false,
